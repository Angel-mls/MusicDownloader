...
class Download:
    ...
    def qqMusic(self, url, path, FileName):
        ...
        DownloadUrl = re.findall(
            "http://aqqmusic.+m4a.+fromtag=\d{1,10}", html)[0]
        print(DownloadUrl)
...
class Gui:  # Gui类，实现图形化页面
    def __init__(self):
        ...
        self.UrlDict = {f"url{i}": [
            "", f"music{randint(0,1000)}{randint(0, 1000)}", ""] for i in range(1, 11)}
            ...
            for music in self.UrlDict:
                if self.UrlDict[music][2] == "QQ音乐":
                    ...
                    if self.UrlDict[music][0] != "":
                        ...
                        FileName = self.UrlDict[music][1]
                        path = self.path
                        D.neteaseCloudMusic(url, path, FileName)
                    else:
                        continue
